...
CLOSURE *eval_cbv(CLOSURE *clos)
{
  ...
  switch(exp->kind)
  {
    ...
    {
      //printf("COND\n");
      result = eval_cbv(make_clos(exp->cond,env,index));
      if(result->ast->kind == CONST)
      {
        int value = result->ast->value;
        if(value != 0)
        {
          result = eval_cbv(make_clos(exp->lchild,env,index));
          // free_clos(clos);
          return result;
        }
        else
        {
          result = eval_cbv(make_clos(exp->rchild,env,index));
          // free_clos(clos);
          return result;        
        } 
      }       
    ...
    case APP:
    {
      //printf("APP\n");
      result = eval_cbv(make_clos(exp->lchild,env,index));
      result2 = eval_cbv(make_clos(exp->rchild,env,index));
      if(result->ast->kind == ABS)
      {
        exp = result->ast->rchild;
        env = make_list(result2,result->env);
        result =  eval_cbv(make_clos(exp,env,result->index));
        return result;
      }
      // free_clos(clos); 
    }
...
CBN_ENV *eval_cbn(CBN_ENV *clos)
{
  ...
  switch(exp->kind)
  {
    ...
    case COND:
    { 
      clos = eval_cbn(make_cbn_env(make_clos(exp->cond,env,index),stack));
      if(clos->cbn_env->ast->kind == CONST)
      {
        int value = clos->cbn_env->ast->value;
        if(value != 0)
    ...
    case APP:
    {
      return eval_cbn(make_cbn_env(make_clos(exp->lchild, env, index), make_list(make_clos(exp->rchild,env,index), stack)));
    }
    default:
    {
      printf("Error!\n");
