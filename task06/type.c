...
int count_new_env = 0, count_free_list = 0;
   ...
   switch(t1->kind)
   {
    ...
    case Arrow:
    {
      ...
      switch(t2->kind)
      {
        ...
        case Arrow:
        {
          if(unify(get_left(t1), get_left(t2)) && unify(get_right(t1), get_right(t2)))
            return 1;
          else 
            return 0;
        }
...
Type_ptr typing (Var_list_ptr abs, AST *t, int top)
{
  ...
  switch(t->kind)
  {
    ...
    case VAR: 
    {
      temp = get_n_th(abs, t->value, top);
      break;
    }
    case ABS:
    {
      temp = make_vartype(); // the left side of ABS(variable kind)
      temp2 = typing(add_var_list(temp, list_copy(abs)),t->rchild,top+1); //typing the abstraction body
      if(temp !=NULL && temp2 !=NULL)
        temp = make_arrowtype(temp, temp2);
      else 
        temp = NULL;
      break;
    }
    case COND:
    {
      temp = typing(list_copy(abs), t->cond,top); //typing the condition
      temp1 = typing(list_copy(abs), t->lchild,top);
      temp2 = typing(list_copy(abs), t->rchild,top);
      break;
    }
    case APP:
    {
      temp = typing(list_copy(abs),t->lchild,top); //typing the left of APP(application function body)
      if(temp == NULL) break;
      temp2 = typing(list_copy(abs),t->rchild,top);
      if(temp2 == NULL)
      {
        ...
        switch (temp->kind)
      {
        case Int: 
        {
           temp = NULL; // const type can't be the application funcation body
           break;
        }
        case Typevar: 
        {
          
          temp = get_instance(temp); // if the left of the application is vairiable, turning it to a arrow type
          if(unify(temp->left,temp2))
            temp = temp->right;
          else
            temp = NULL;
          break;
  ...
  if (stdin == yyin)
  {
    printf("typing step = %d and current top = %d \n", ++step, top);
    print_abs(abs);
    print_env();
    print_expression(t,stdout);
    printf(" |== ");
    print_type_debug(temp);
    printf("\n");
  }
  free_list(abs);
  return temp;
...
Type_ptr type_copy(Type_ptr t)
{
  ...
  case Arrow: return make_arrowtype(type_copy(t -> left),
            type_copy(t -> right));
  }
}
